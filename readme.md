...
## Recommended Workflow
  ...
  5. Close the notebook when you're done and in the jupyter tab (with the files listed) check the box next to the ipynb file and choose "shut down" from the little menu above (orangish). This will free up some memory.
  6. Make a copy of the notebook and change the filename of the copy to "explorer_experiment_C2020.....Shank2.ipynb"
     - `chan_num = [64-128]` for Shank2
     - Kernel -> Restart Kernel and Run All (or manually run the boxes in order)
  7. Repeat these steps as needed to cover the full length recording from all channels.
  8. After the files have been filtered in explorer, load the filtered h5 files into h5_spikeviewer to tweak the threshold for spike detection on the different channels or to make figures. It analyzes just 1 second of data.
  9. Once you know the correct threshold, change the user input in h5_spikedetector for full-length analysis.
  10. Once you have finished analyzing each of these files, transfer them for storage or they will eat up disk space. In testing, a 2 min, 64 channel recording from the cerebellum with a threshold of 2 resulted in a 1.5GB events csv. That is a HUGE amount of data, so don't get too backlogged.
